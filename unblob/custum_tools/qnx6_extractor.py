...
class QNX6FS:
	...
	def __init__(self, source, output_directory):
		self.TARGET_ = source
		self.OUTPUT_ = output_directory
	...
	def parseINODE(self,superBlock,PartitionID):
				...
				for i in self.DirTree:
					self.dumpfile(i, self.OUTPUT_ ,superBlock['blocksize'],superBlock['blks_offset'],PartitionID)
	...
	def isBlockEmpty(self,blocknumber,blksize,blksOffset):
		...
		Data = ''.join([x for x in Data if ord(x) > 0]) # type: ignore
		return len(Data) < 1
	...
	def getSnippet(self,blocknumber,blksize,blksOffset):
		...
		Data = re.sub(r'[^\x00-\x7F]+','', Data) # type: ignore
			...
			if level == 0:
				if self.checkQNX6blkptr(ptrs[i]):
					if ptrs[i] != 0xffffffff and ptrs[i] != 0x0:
						...
						if (InodeDataEntry['size'] - io.tell()) >= 1024: # type: ignore
							DATABUFF += self.fileIO.read(blksize)
						else:
							DATABUFF += self.fileIO.read((InodeDataEntry['size'] - io.tell())) # type: ignore
			else:		
				self.fileIO.seek(ptrs[i])
				...
				self.batchProcessPTRS(level2_PTRS,InodeDataEntry,level-1,blksize,blkOffset,path,io) # type: ignore

		if level == 0:
			io.write(DATABUFF) # type: ignore
...
def main():
	if os.path.exists(sys.argv[1]):
		Q6 = QNX6FS(sys.argv[1], sys.argv[2])
		...
		print("- [-] QNX 6.5.1 TDB")
		print("- ")
		print("- Usage: qnx6_extractor.py q_system.img output_dir")
		print("------------------------------------------------------------")
